<?php
    ...
    function peticion () {
         ...
         if ( !isset( getallheaders()["Authorization"] ) || isset( getallheaders()["Authorization"] ) && getallheaders()["Authorization"] != apiKey() ) {
            ...
            $tablaPublica = explode ( "/" , $_SERVER['REQUEST_URI'] );
        
            $tablaPublica = array_filter ( $tablaPublica );
            
            if ( isset ($tablaPublica[1]) ) {
                $tablaPublica = explode("?",$tablaPublica[1]);
            }
            
            if ( isset ( $tablaPublica[0] ) ) {
            
                if ( in_array($tablaPublica[0],accesoPublico ()) ) {
                    
                    $lab2 = new Adaptowebs;
                    
                    $lab2 -> set_db ( "your dbName" );
                    $lab2 -> set_host ( "your host" );
                    $lab2 -> set_charset ( "utf8mb4" );
                    $lab2 -> set_user ( "your user" );
                    $lab2 -> set_pass ( "your password" );
                    
                    $conexion2 = $lab2 -> pdo ( $lab2 );
                    
                    $lab2 -> getData ( $tablaPublica[0] , select () , orderby () , ordermode () , startat () , endat () , $conexion2 , $lab2 );
